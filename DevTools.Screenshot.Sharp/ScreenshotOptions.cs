...
public sealed class ScreenshotOptions
{
    ...
    public static TimeSpan DefaultDelay { get; } = TimeSpan.FromMilliseconds(DefaultDelayMilliseconds);
    ...
    public void EnsureEnabled()
    {
        if (!IsEnabled)
        {
            throw new InvalidOperationException(EnsureEnabledMessage);
        }
    }
    ...
    public string RequireOutputPath() =>
        !string.IsNullOrWhiteSpace(OutputPath)
            ? OutputPath!
            : throw new InvalidOperationException(RequireOutputPathMessage);
}
