...
public sealed class WinUiScreenshot : IScreenshot
{
    ...
    private const int MaxReadyAttempts = 100;
    private const int ReadyPollDelayMilliseconds = 20;

    private const string NoContentMessage = "Main window has no content to capture.";
    private const string NoXamlRootMessage = "Main window content has no XamlRoot.";
    private const string NoMeasurableAreaMessage = "Window has no measurable area to capture.";
    private const string ReadyTimeoutMessage = "Timed out waiting for the main window to become ready for capture.";
        ...
        CancellationToken cancellationToken)
    {
        if (window.Content is not FrameworkElement content)
            throw new InvalidOperationException(NoContentMessage);
        ...
        if (content.ActualWidth < MinMeasurableDimension || content.ActualHeight < MinMeasurableDimension)
        ...
        {
            await CopyToClipboardAsync(pixels, rtb.PixelWidth, rtb.PixelHeight, BaseDpi * scale, cancellationToken);
        ...
        await SavePngAsync(outputPath, pixels, rtb.PixelWidth, rtb.PixelHeight, BaseDpi * scale, cancellationToken);
    ...
    private static async Task WaitForLayoutAndRenderAsync(FrameworkElement content, CancellationToken cancellationToken)
    {
        for (var attempt = 0; attempt < MaxReadyAttempts; attempt++)
        {
            ...
            await Task.Delay(ReadyPollDelayMilliseconds, cancellationToken);
        }

        throw new InvalidOperationException(ReadyTimeoutMessage);
    }
