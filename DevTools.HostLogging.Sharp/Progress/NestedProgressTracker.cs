namespace DevTools.HostLogging.Sharp.Progress;

internal sealed class NestedProgressTracker
{
    #region Error messages

    private const string StepCountOutOfRangeMessage = "Step count must be greater than zero.";
    private const string NestedProgressAfterCompleteMessage = "Cannot begin a nested progress phase after the current phase is complete.";
    private const string NoActiveProgressMessage = "No active progress phase. Call BeginProgress before CompleteStep.";
    private const string TooManyCompleteStepsMessage = "CompleteStep called more times than declared steps.";
    ...
    public ProgressSnapshot BeginProgress(int totalSteps)
    {
        if (totalSteps <= 0)
        {
            throw new ArgumentOutOfRangeException(nameof(totalSteps), totalSteps, StepCountOutOfRangeMessage);
        }
        ...
        {
            throw new InvalidOperationException(NestedProgressAfterCompleteMessage);
        }
    ...
    public ProgressSnapshot CompleteStep()
    {
        if (_scopes.Count == 0)
        {
            throw new InvalidOperationException(NoActiveProgressMessage);
        }
    ...
    private sealed class Scope(int totalSteps, double weight)
    {
        ...
        public void CompleteStep()
        {
            if (IsCompleted)
            {
                throw new InvalidOperationException(TooManyCompleteStepsMessage);
            }
