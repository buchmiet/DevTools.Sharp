...
{
    #region Attach protocol

    internal const string CloseControlCommand = "close";
    private const string AttachPipeServer = ".";
    private const int ConnectTimeoutMilliseconds = 10_000;
    private const int DisposeWaitSeconds = 1;
    private const string SessionAttachedMessage = "Startup session attached.";
    ...
    public void Close()
    {
        ...
        _outbox.Writer.TryWrite(StartupWireMessage.CreateControl(CloseControlCommand));
    ...
    public void Dispose()
    {
        ...
        try
        {
            _forwarder?.Wait(TimeSpan.FromSeconds(DisposeWaitSeconds));
        }
        catch
        {
        }
    ...
    private static string? ResolveAttachName(string? attachName)
    {
        ...
        return Environment.GetEnvironmentVariable(HostLog.RemoteAttachVariable);
    }
    ...
    private async Task ForwardAsync(string attachName, CancellationToken cancellationToken)
    {
        try
        {
            ...
            await channel.ConnectAsync(ConnectTimeoutMilliseconds, cancellationToken).ConfigureAwait(false);
